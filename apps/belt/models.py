...
class ItemManager(models.Manager):
	...
	def item_validator(self,postData):
		...
		if len(postData['product']) < 3:
			errors['product_error'] = "item name field cannot be less than 3 character"
		elif len(postData['product']) < 0:
			errors['product_error'] = "item name cannot be empty"
		return errors
...
class Item(models.Model):
	...
	def __unicode__(self):
		return "id : " + str(self.id) + ", item_name : " + self.item_name + \
		" , uploader : " + str(self.uploader) + " , liked_users : " + str(self.liked_users) + \
		" , liked_items : " + str(self.liked_items)
